    ...
    while (
        ...
        if np.random.random() <= epsilon:
            ...
            if sample_actions:
                ...
                action = (
                    (probs / probs.sum(axis=1).reshape(-1, 1)).cumsum(axis=1)
                    < np.random.random((probs.shape[0], 1))
                ).sum(axis=1)
                action[action == 4] = 3
            else:
                # get action with best q value
...
def visualize_game(
    ...
    if animation:
        fig, axs = plt.subplots(
            1, 1, figsize=(board_size // 2 + 1, board_size // 2 + 1)
        )
            ...
            for j in range(ncols):
                ...
                if idx < len(game_images):
                    ...
                    axs[i, j] = anim_frames_func(
                        game_images[idx], axs[i, j], color_map, food_count, qvalues
                    )
                else:
                    fig.delaxes(axs[i, j])
...
def anim_frames_func(board_time, axs, color_map, food_count, qvalues):
    ...
    title = "time:{:d}, score:{:d}\n{:.2f} {:.2f} {:.2f} {:.2f}".format(
        time, food_count[time], *qvalues[time]
    )
    axs.set_title(title)
    plt.tight_layout()
    return axs
...
def plot_logs(data, title="Rewards and Loss Curve for Agent", loss_titles=["Loss"]):
    """
        ...
        if data["loss"].dtype == "O":
            ...
            for i in range(loss_count):
                data["loss_{:d}".format(i)] = data["loss"].apply(
                    lambda x: float(x.replace("[", "").replace("]", "").split(",")[i])
                )
    ...
    fig, axs = plt.subplots(
        1 + loss_count + 1 if "length_mean" in data.columns else 0, 1, figsize=(8, 8)
    )
