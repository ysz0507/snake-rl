...
layer_num = 1
model_temp = Model(
    inputs=agent._model.input, outputs=agent._model.layers[layer_num].output
)
output_temp = model_temp.predict(s.reshape(1, board_size, board_size, frames))[
    0, :, :, :
]
...
fig.savefig(
    "images/weight_visual_{:s}_{:04d}_conv{:d}.png".format(
        version, iteration, layer_num
    ),
    dpi=72,
    bbox_inches="tight",
)
...
while not done:
    ...
    fig.savefig(
        "images/weight_visual_{:s}_{:02d}.png".format(version, t),
        dpi=72,
        bbox_inches="tight",
    )
    # plt.show()
...
os.system(
    'ffmpeg -y -framerate 1 -pattern_type sequence -i "images/weight_visual_{:s}_%02d.png" \
          -c:v libx264 -pix_fmt gray images/weight_visual_{:s}_{:04d}_conv{:d}.mp4'.format(
        version, version, iteration, layer_num
    )
)
