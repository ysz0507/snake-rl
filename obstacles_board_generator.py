...
while index < total_boards:
    ...
    if index // track_index > prev_index // track_index:
        ...
        print(
            "Generated a total of {:d} boards in {:3d}hrs {:2d}mins {:2d}secs {:.3f}ms.".format(
                index, hrs, mins, secs, msecs
            )
        )
    prev_index = index
    ...
    while trial_counter < board_size**2:
        ...
        if not connected:
            ...
            duplicate = False
            if index > 0:
                if (obstacles_board[:index, :, :] == board).sum(
                    (1, 2)
                ).max() == board_size**2:
                    duplicate = True
            if duplicate:
...
hrs, mins, secs, msecs = time_calculate(start_time, end_time)
print(
    "Generated a total of {:d} boards in {:3d}hrs {:2d}mins {:2d}secs {:.3f}ms. Saving to disk".format(
        index, hrs, mins, secs, msecs
    )
)
